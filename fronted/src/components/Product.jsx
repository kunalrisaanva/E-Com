  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/products/product?q=${category}&sort=${sortBy}&limit=${showCount}`
        );
        const { data } = response;
          ...
          <div className=" mt-[47px] w-full ">
            <div className="flex  justify-between bg-[#F6F7F8] items-center h-[56px]  ">
              <div className="flex items-center gap-10 pl-[22px] ">
                ...
                className="bg-[#F1F3F4] w-full h-full rounded-md border-none focus:outline-none"
              >
